defmodule WhimsyWeb.UserSessionControllerTest do
  use WhimsyWeb.ConnCase, async: true

  import Whimsy.AccountsFixtures
  ...
  describe "POST /users/log-in - email and password" do
    test "logs the user in", %{conn: conn, user: user} do
      ...
      # Now do a logged in request and assert on the home page
      conn = get(conn, ~p"/")
      response = html_response(conn, 200)
      assert response =~ "WHIMSY"
    end
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_whimsy_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert Whimsy.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
